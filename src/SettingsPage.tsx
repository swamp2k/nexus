...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    function changedIntegrations(event: Event) {
      const next = (event as CustomEvent<IntegrationMap>).detail;
      if (next) setIntegrations({ ...DEFAULT_INTEGRATIONS, ...next });
    }
    window.addEventListener("nexus-integrations-changed", changedIntegrations);
    return () => window.removeEventListener("nexus-integrations-changed", changedIntegrations);
  }, []);
  ...
  return <section className="settings-page" aria-labelledby="settings-heading">
    ...
    {integrations.garmin && <details className="settings-card settings-collapsible settings-component-wrapper"><summary className="settings-card-heading"><div><p className="section-label">Datakilde</p><h2>Garmin</h2></div><span className="settings-icon" aria-hidden="true">⌖</span></summary><GarminImportSettings /></details>}
